...
|---|---|---|
| 0.1.6 | July 22, 2026 | §7.2 adds a day-one publication-halt rule for zero coverage: if any constituent of a sub-index records zero clean MOC-window observations across all included mappings on a trading day, that sub-index is halted for the day (no value computed or published), independently of and below the §7.4 impairment thresholds, with the five-day escalation clock running concurrently from day one. 2026-06-24 reclassified as halted under §12.1 after a Together provider outage produced zero clean observations for two constituents and mapping-level dropout across six more, yielding composition-artifact published values (17 contributing constituents summed against a 19-constituent divisor); a re-anchor-on-survivors correction was computed and rejected as itself a composition artifact; prior values preserved in `vipi_corrections`, retroactive reclassification disclosed (§12.1.2, VIPI_v0.1.6_errata_001). No change to the index family, inclusion criteria, weighting, the §7.4 thresholds, or the §8 calculation formula. |
| 0.1.5 | June 11, 2026 | §12.2.1 added — retrospective halt record for the three spurious halts on 2026-05-27, 2026-06-12, and 2026-06-13 (`all_constituents_zero_observations`), root-caused to an upstream single-dominant-provider mis-calibration in the price-aggregation worker's whole-snapshot quality-flagging heuristic. Fix disclosed and verified in production (per-provider attribution with a 10% residual-feed threshold; test suite pinned; 2026-06-16 DeepInfra single-cycle outage correctly classified `clean`). Halted days retained on the record consistent with §12.2's no-backfill principle; halt-state monitoring control committed to. No change to the index family, inclusion criteria, weighting, halt rules, or the §8 calculation formula. |
...
**Day-one publication halt for zero coverage (v0.1.6).** If on any trading day one or more basket constituents of a sub-index record zero clean MOC-window observations across all included mappings, publication of that sub-index is **halted for that day** (halt reason `provider_outage_zero_coverage`, or `constituent_zero_coverage` as applicable); no value is computed or published. Sub-indices whose constituents all retain coverage publish normally. The five-day escalation clock defined in the paragraphs above runs concurrently from day one, so a transient outage that resolves before day 5 triggers no structural removal while nonetheless suppressing publication on each affected day. This halt trigger operates **independently of, and below, the §7.4 impairment thresholds**: a single zero-coverage constituent halts the affected sub-index even though it does not reach §7.4's ">30% of constituents impaired" or ">3 constituents simultaneously" thresholds. The rationale is that a constituent recording zero clean observations cannot contribute to the numerator; retaining its 1/N weight against an unadjusted divisor would let a composition change register as price movement, the outcome §8.1's divisor mechanism exists to prevent. This paragraph specifies the first day of a transient zero-coverage event, which §7.2's five-day escalation and §9.3's structural-removal condition did not previously address.
...
#### 12.1.2. Correction Record — 2026-06-24 Provider-Outage Zero Coverage (v0.1.6)

**Facts.** On 2026-06-24 the Together provider feed produced zero clean MOC-window observations (15:55–16:05 UTC). Two constituents — `vcmi:llama-3.1-405b` and `vcmi:llama-3.1-70b` (the nvidia/nemotron mapping) — had zero clean observations from **all** of their included mappings; six further surviving constituents lost Together as a medium-confidence fallback mapping. vipi-cron excluded the null-close constituents from the numerator while retaining 1/N weights and carrying the prior 19-constituent divisor forward, so **17 contributing constituents were summed against a 19-constituent divisor**. The published VIPI and VIPI-Open values for that day were therefore composition artifacts, not price observations.

Prior published values (`halted = false`, methodology `vipi-0.1.5`), all eight now corrected:

| Sub-index | Value kind | Prior published value |
|---|---|---|
...
Reference (adjacent closes, unchanged and identical to each other): 2026-06-23 and 2026-06-25 — VIPI blended 100.61440480082791, VIPI-Open blended 100.57205371459115. `vipi_closed` was unaffected on 2026-06-24 (all four value kinds at or ≈ 100.00, constituent count 6): its constituents retained coverage.

**The methodology gap.** The day-one transient zero-coverage case was unspecified in v0.1.5. §7.2's sustained-loss escalation covers only losses persisting ≥ 5 consecutive trading days, and §9.3's fourth emergency-removal condition requires a structural classification; neither addressed the **first day** of a transient outage. The default vipi-cron arithmetic — numerator over surviving constituents, unadjusted divisor, unchanged 1/N weights — filled that gap with a composition-artifact value. §7.2's new "Day-one publication halt for zero coverage (v0.1.6)" closes the gap prospectively. Coverage recovered on 2026-06-25 and published values self-restored.

**The rejected alternative.** A re-anchor-on-survivors correction (recomputing the divisor on the 17 surviving constituents at the 2026-06-23 close) was computed and **rejected** by the Administrator. It yields VIPI blended 77.29619426028977 — a residual −23% step relative to the 2026-06-23 close that is itself pure mapping-composition artifact, not price movement: all DeepInfra survivor prices were bit-identical from 2026-06-23 to 2026-06-24, and the re-anchored `best_blended` corrected values equal the 2026-06-23 closes **exactly**, proving no price movement occurred on 2026-06-24. Publishing the re-anchored series would let a composition change register as price movement, contrary to §8.1's purpose. There is no defensible reconstructed value to publish.

**Resolution — reclassification to halted.** Under the v0.1.6 §7.2 day-one zero-coverage rule, 2026-06-24 is reclassified as a **publication halt** for VIPI and VIPI-Open, retroactively, with the retroactivity explicitly disclosed here. The eight daily rows are set to halted (`value` NULL, `divisor` NULL, `constituent_count` 0, `halted` true, `halt_reason` `provider_outage_zero_coverage`, methodology `vipi-0.1.6`); the prior published values above are preserved in `vipi_corrections` under `reason_class = calculation_error`. `vipi_closed` is unchanged. Consistent with the no-backfill principle, no value is reconstructed: the day is disclosed as uncomputable-as-specified. Full diagnostic: VIPI_v0.1.6_errata_001.

**Relation to the §12.1.1 statement on retroactive halts.** §12.1.1 states, for the 2026-06-05 event, that "a retroactive halt would itself depart from the published rules." That holds for **that** event under the rules **then** in force: on 2026-06-05 the published values were the correct output of the then-specified calculation for a single below-threshold impaired constituent, so manufacturing a retroactive halt of correct values would have departed from the published rules. This 2026-06-24 correction is different in kind. It is made under §12.1, with the v0.1.6 rule now on the record and full disclosure, and it replaces **mis-stated** values — composition artifacts that were never a valid index observation — with the halt that the now-published rule specifies. The distinction is between manufacturing a retroactive halt of correct values (impermissible, §12.1.1) and correcting mis-stated values to a halt (this record).
...
*VIPI v0.1.6 is a draft methodology. It will evolve through private-phase review before public launch. Feedback from academic, industry, and regulatory reviewers is encouraged and will shape the v1.0 methodology. Substantive changes across versions are documented in the changelog.*
